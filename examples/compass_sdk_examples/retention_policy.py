...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="""
Manage retention policies for a Compass index.
...
def main():
    ...
    if args.action == "set":
        ...
        client.set_retention_policy(index_name=index_name, retention_policy=policy)
        print("\nRetention policy set successfully.")

    elif args.action == "get":
        print(f"Getting retention policy for index '{index_name}'...")

        policy = client.get_retention_policy(index_name=index_name)
        ...
        client.delete_retention_policy(index_name=index_name)
